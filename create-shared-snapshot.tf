resource "aws_db_cluster_snapshot" "this" {
  ...
  db_cluster_snapshot_identifier = var.db_cluster_snapshot_identifier != null ? var.db_cluster_snapshot_identifier : substr("${aws_rds_cluster.this[0].id}-v${var.shared_snapshot_version}", 0, 63)
